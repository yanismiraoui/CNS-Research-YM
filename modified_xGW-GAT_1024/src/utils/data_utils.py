...
def process_pair(data, i, j, method, args):
    ...
    elif method in ("tan", "node", "eigen", "close", "concat_orig", "concat_scale"):
        dist = spd.transp(
...
def create_features(data, score, args, method="eigen", num_processes=4):
    data_dict = {}
    score_dict = {}
    spd = SPD.SPD(args.num_nodes)
    ...
    with multiprocessing.Pool(processes=num_processes) as pool:
        ...
        total_tasks = data.shape[2] * (data.shape[2] - 1) // 2  # Total number of tasks
        progress_bar = tqdm(total=total_tasks, unit="pair")

        for i in range(data.shape[2]):
            for j in range(i + 1, data.shape[2]):
                result = pool.apply_async(process_pair, args=(data, i, j, method, args))
